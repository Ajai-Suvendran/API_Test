    ...
    foreach var [key, value] in paths.entries() {
        ...
        foreach int i in 0 ... parameters.length() - 1 {
            ...
            if sanitizedParamName.includes("ByKey") {
                ...
                string nameCheck = sanitizedParamName.substring(0,startIndex).concat(sanitizedParamName.substring(startIndex+5));
                EnumSchema? schema = selectedSchemas[nameCheck];
                if schema is EnumSchema{
                    ...
                    if count == item.length(){
                        sanitizedParamName = nameCheck;
                    }
                }
            }
            if sanitizedParamName.includes("Of"){
                int startIndex = <int>sanitizedParamName.indexOf("Of");
                string nameCheck = "A_".concat(sanitizedParamName.substring(0,startIndex));
                if sanitizedParamName.includes("Select"){
                    nameCheck = nameCheck.concat("SelectOptions");
                }
                else if sanitizedParamName.includes("Expand") {
                    nameCheck = nameCheck.concat("ExpandOptions");
                }
                else if sanitizedParamName.includes("Orderby") {
                    nameCheck = nameCheck.concat("OrderByOptions");
                }
                else if sanitizedParamName.includes("Filter") {
                    nameCheck = nameCheck.concat("FilterOptions");
                }
                else {
                    nameCheck = nameCheck.concat("SearchOptions");
                }
                EnumSchema? schema = selectedSchemas[nameCheck];
                if schema is EnumSchema{
                    string[] item = schema.items.'enum;
