...
function sanitizeEnumParamters(string specPath) returns error? {
    ...
    foreach var [key, value] in paths.entries() {
        ...
        foreach int i in 0 ... parameters.length() - 1 {
            ...
            [string, string, string] sanitizedParamName = getSanitizedParameterName(key, param.name ?: "");
...
function getSanitizedParameterName(string key, string paramName) returns [string, string, string] {
